...
(test 'list-tabulate:factorial
	  (list-tabulate 7 (lambda (position)
						 (do ((product 1 (* product multiplier))
...
(test 'list*:one-argument
	  (list* 'Columba)
	  (lambda (result) (eq? result 'Columba)))

(test 'list*:two-arguments
	  (list* 'Corvus 'Crater)
	  (lambda (result) (equal? result '(Corvus . Crater))))

(test 'list*:many-arguments
	  (list* 'Crux 'Cygnus 'Delphinus 'Dorado 'Draco)
	  (lambda (result)
		(equal? result '(Crux Cygnus Delphinus Dorado . Draco))))

(test 'list*:last-argument-null
	  (list* 'Equuleus 'Fornax '())
	  (lambda (result) (equal? result '(Equuleus Fornax))))

(let ((base '(Gemini Grus)))
  (test 'list*:last-argument-non-empty-list
		(list* 'Hercules 'Horologium 'Hydra 'Hydrus base)
		(lambda (result)
...
(let ((pair '(manx . siamese)))
  (test 'last-pair:of-improper-lsit
...
(define reverse-append append-reverse)
(define reverse-append! append-reverse!)

(test 'reverse-append:first-argument-null
	  (reverse-append '() '(Typhon unicorn vampire werewolf))
	  (lambda (result)
		(equal? result '(Typhon unicorn vampire werewolf))))

(test 'reverse-append:second-argument-null
	  (reverse-append '(windigo wivern xiphopagus yeti zombie) '())
	  (lambda (result)
		(equal? result '(zombie yeti xiphopagus wivern windigo))))

(test 'reverse-append:both-arguments-null
	  (reverse-append '() '())
	  null?)

(test 'reverse-append:neither-argument-null
	  (reverse-append '(Afghanistan Albania Algeria Andorra)
...
(test 'reverse-append!:second-argument-null
	  (reverse-append! (list 'Bahrain 'Bangladesh 'Barbados
							 'Belarus 'Belgium)
					   (list))
	  (lambda (result)
		(equal? result
...
(test 'reverse-append!:neither-argument-null
	  (reverse-append! (list 'Belize 'Benin 'Bhutan 'Bolivia)
...
(define unfold/tail unfold-right)
...
(test 'foldl:one-null-list
	  (foldl (lambda (alpha beta) (* alpha (+ beta 1))) 13 '())
	  (lambda (result) (= result 13)))

(test 'foldl:one-singleton-list
	  (foldl (lambda (alpha beta) (* alpha (+ beta 1))) 13 '(15))
	  (lambda (result) (= result 210)))

(test 'foldl:one-longer-list
	  (foldl (lambda (alpha beta) (* alpha (+ beta 1)))
			 13
			 '(15 17 19 21 23))
	  (lambda (result) (= result 32927582)))

(test 'foldl:several-null-lists
	  (foldl vector 'Chad '() '() '() '() '())
	  (lambda (result) (eq? result 'Chad)))

(test 'foldl:several-singleton-lists
	  (foldl vector 'Chile '(China) '(Colombia) '(Comoros) '(Congo)
			 '(Croatia))
	  (lambda (result)
		(equal? result
				'#(China Colombia Comoros Congo Croatia Chile))))

(test 'foldl:several-longer-lists
	  (foldl (lambda (alpha beta gamma delta epsilon zeta)
...
(test 'foldl:lists-of-different-lengths
	  (foldl (lambda (alpha beta gamma delta)
...
(test 'foldr:one-null-list
	  (foldr (lambda (alpha beta) (* alpha (+ beta 1))) 13 '())
	  (lambda (result) (= result 13)))

(test 'foldr:one-singleton-list
	  (foldr (lambda (alpha beta) (* alpha (+ beta 1))) 13 '(15))
	  (lambda (result) (= result 210)))

(test 'foldr:one-longer-list
	  (foldr (lambda (alpha beta) (* alpha (+ beta 1)))
			 13
			 '(15 17 19 21 23))
	  (lambda (result) (= result 32868750)))

(test 'foldr:several-null-lists
	  (foldr vector 'Lebanon '() '() '() '() '())
	  (lambda (result) (eq? result 'Lebanon)))

(test 'foldr:several-singleton-lists
	  (foldr vector 'Lesotho '(Liberia) '(Libya) '(Liechtenstein)
			 '(Lithuania) '(Luxembourg))
	  (lambda (result)
		(equal? result '#(Liberia Libya Liechtenstein Lithuania
								 Luxembourg Lesotho))))

(test 'foldr:several-longer-lists
	  (foldr (lambda (alpha beta gamma delta epsilon zeta)
			   (cons (vector alpha beta gamma delta epsilon) zeta))
			 '()
...
(test 'foldr:lists-of-different-lengths
	  (foldr (lambda (alpha beta gamma delta)
			   (cons (vector alpha beta gamma) delta))
			 '()
...
(define pair-foldl pair-fold)
(define pair-foldr pair-fold-right)
...
(let* ((revappend (lambda (reversend base)
  ...
  (test 'pair-foldl:one-null-list
		(pair-foldl revappend '(Spain Sudan) '())
		(lambda (result) (equal? result '(Spain Sudan))))

  (test 'pair-foldl:one-singleton-list
		(pair-foldl revappend '(Suriname Swaziland) '(Sweden))
		(lambda (result)
		  (equal? result '(Sweden Suriname Swaziland)))) 

  (test 'pair-foldl:one-longer-list
		(pair-foldl revappend
					'(Switzerland Syria)
  ...
  (test 'pair-foldl:several-null-lists
		(pair-foldl revappall '(Tonga Tunisia) '() '() '() '() '())
		(lambda (result) (equal? result '(Tonga Tunisia))))

  (test 'pair-foldl:several-singleton-lists
		(pair-foldl revappall
					'(Turkey Turkmenistan)
					'(Tuvalu)
					'(Uganda)
					'(Ukraine)
  ...
  (test 'pair-foldl:several-longer-lists
		(pair-foldl revappall
					'(Vanuatu Venezuela)
  ...
  (test 'pair-foldl:lists-of-different-lengths
		(pair-foldl revappall
...
(let* ((revappend (lambda (reversend base)
  ...
  (test 'pair-foldr:one-null-list
		(pair-foldr revappend '(Maeterlinck Mahfouz) '())
		(lambda (result) (equal? result '(Maeterlinck Mahfouz))))

  (test 'pair-foldr:one-singleton-list
		(pair-foldr revappend '(Mann Martinson) '(Mauriac))
		(lambda (result)
		  (equal? result '(Mauriac Mann Martinson)))) 

  (test 'pair-foldr:one-longer-list
		(pair-foldr revappend
					'(Milosz Mistral)
  ...
  (test 'pair-foldr:several-null-lists
		(pair-foldr revappall '(Pasternak Paz) '() '() '() '() '())
		(lambda (result) (equal? result '(Pasternak Paz))))

  (test 'pair-foldr:several-singleton-lists
		(pair-foldr revappall
					'(Perse Pirandello)
					'(Pontoppidan)
					'(Quasimodo)
					'(Reymont)
  ...
  (test 'pair-foldr:several-longer-lists
		(pair-foldr revappall
					'(Sachs Sartre)
  ...
  (test 'pair-foldr:lists-of-different-lengths
		(pair-foldr revappall
...
(define reducel reduce)
(define reducer reduce-right)

(test 'reducel:null-list
	  (reducel (lambda (alpha beta) (* alpha (+ beta 1))) 0 '())
	  zero?)

(test 'reducel:singleton-list
	  (reducel (lambda (alpha beta) (* alpha (+ beta 1))) 0 '(25))
	  (lambda (result) (= result 25)))

(test 'reducel:doubleton-list
	  (reducel (lambda (alpha beta) (* alpha (+ beta 1)))
			   0
			   '(27 29))
	  (lambda (result) (= result 812)))

(test 'reducel:longer-list
	  (reducel (lambda (alpha beta) (* alpha (+ beta 1)))
			   0
...
(test 'reducer:null-list
	  (reducer (lambda (alpha beta) (* alpha (+ beta 1))) 0 '())
	  zero?)

(test 'reducer:singleton-list
	  (reducer (lambda (alpha beta) (* alpha (+ beta 1))) 0 '(25))
	  (lambda (result) (= result 25)))

(test 'reducer:doubleton-list
	  (reducer (lambda (alpha beta) (* alpha (+ beta 1)))
			   0
			   '(27 29))
	  (lambda (result) (= result 810)))

(test 'reducer:longer-list
	  (reducer (lambda (alpha beta) (* alpha (+ beta 1)))
			   0
			   '(31 33 35 37 39 41 43))
...
(define (del maybe-= x lis)
  (delete x lis maybe-=))
(define (del! maybe-= x lis)
  (delete! x lis maybe-=))

(test 'del:null-list
	  (del (lambda (x y) #t) 'Earling '())
	  null?)

(test 'del:singleton-list
	  (del (lambda (x y) #f) 'Earlville '(Early))
	  (lambda (result) (equal? result '(Early))))

(test 'del:all-elements-removed
	  (del (lambda (x y) #t)
		   'Eckards
		   '(Eddyville Edgewood Edinburg Edmore Edna))
	  null?)

(test 'del:some-elements-removed
	  (del (lambda (x y) (even? (+ x y)))
		   489
		   '(490 491 492 493 494))
	  (lambda (result) (equal? result '(490 492 494))))

(test 'del:no-elements-removed
	  (del (lambda (x y) #f)
		   'Egan
...
(test 'del!:null-list
	  (del! (lambda (x y) #t) 'Eldora (list))
	  null?)

(test 'del!:singleton-list
	  (del! (lambda (x y) #f) 'Eldorado (list 'Eldridge))
...
(define (delq x lis) (delete x lis eq?))
(define (delq! x lis) (delete! x lis eq?))
(define (delv x lis) (delete x lis eqv?))
(define (delv! x lis) (delete! x lis eqv?))
...
(define (del-duplicates elt= lis) (delete-duplicates lis elt=))
(define (del-duplicates! elt= lis) (delete-duplicates! lis elt=))
(define (delq-duplicates lis) (delete-duplicates lis eq?))
(define (delq-duplicates! lis) (delete-duplicates! lis eq?))
(define (delv-duplicates lis) (delete-duplicates lis eqv?))
(define (delv-duplicates! lis) (delete-duplicates! lis eqv?))
...
(define (mem elt= x lis) (member x lis elt=))
...
(define (ass elt= x lis) (assoc x lis elt=))
...
(test 'alist-delete:some-elements-removed
	  (alist-delete (lambda (x y) (odd? (+ x y)))
					561
					'((562 . 563)
					  (565 . 564)
					  (566 . 567)
...
(test 'alist-delete!:null-list
	  (alist-delete! (lambda (x y) #t) 'Mitchell '())
	  null?)
...
(test 'alist-delete!:some-elements-removed
	  (alist-delete! (lambda (x y) (even? (+ x y)))
					 572
					 (list (cons 573 574)
						   (cons 576 575)
						   (cons 577 578)
...
(define (del-ass elt= x lis) (alist-delete x lis elt=))
(define (del-ass! elt= x lis) (alist-delete! x lis elt=))
(define (del-assq x lis) (alist-delete x lis eq?))
(define (del-assq! x lis) (alist-delete! x lis eq?))
(define (del-assv x lis) (alist-delete x lis eqv?))
(define (del-assv! x lis) (alist-delete! x lis eqv?))
